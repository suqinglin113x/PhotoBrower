...
@objc protocol AdvertScrollViewDelegate:NSObjectProtocol {
    //设置协议方法
    func advertScrollView(advertScrollView : AdvertScrollView,index : NSInteger)
    @objc optional func advertScrollViewTo(index: NSInteger)
}
...
class AdvertScrollView : UIView {
    ...
    @objc fileprivate func beginUpdateUI() {
        ...
        if let delegate = delegate {
            delegate.advertScrollViewTo?(index: nextItem)
        }
        // 3、通过动画滚动到下一个位置
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if advertScrollViewStyle == .more {
            ...
            do{
                if imageArr.count > 0 {
                    ...
                    if topImagePath.hasPrefix("http") == true {
                        cell.topSignImageView.sd_setImage(with: URL.init(string: topImagePath))
                    }else{
            ...
            do {
                if (bottomSignImages?.count)! > 0{
                    ...
                    if imagePath.hasPrefix("http") == true {
                        cell.bottomSignImageView.sd_setImage(with: URL.init(string: imagePath))
                    }else{
            ...
            if imageArr.count > 0{
                ...
                if imagePath.hasPrefix("http") == true {
                    cell.signImageView.sd_setImage(with: URL.init(string: imagePath))
                }else{
